...
- **音声で復習（オフライン・無料）**: 当日間違えた問題／全体でよく間違える問題の解説を、**端末内蔵の音声合成（Web Speech API）**で読み上げ。外部API・APIキー・通信は不要で、追加費用もゼロ。原稿はアプリ内の解説データから自動で組み立て（分野→問題→正解→あなたの解答→正解の理由→誤答の理由の順）、読み上げ中の行をハイライト表示。速度（0.8〜1.5倍）・声の種類・問題文の読み上げ有無を設定可能。ホーム「音声で復習」、下部メニュー「音声復習」、セッション終了画面から起動。
- **まとめノート（HTML）**: 項目（カテゴリ）ごとに「間違いが多い問題」または「その項目の全問題」を、**見やすいHTML資料**として生成。問題・選択肢（正解／前回選んだ誤答を色分け）・正解の理由・ほかの選択肢が違う理由・自分のメモを1枚にまとめ、目次と正答率サマリつき。アプリ内プレビューのほか、**HTMLファイルで保存**（オフラインで開ける）・**印刷／PDF保存**・「この範囲を解く」に対応。
...
読み上げは **ブラウザ／OS 内蔵の音声合成（`speechSynthesis`）** だけで行います。クラウドのTTSや
Claude等のAPIは使いません（そもそもテキスト生成APIは音声を返しません）。そのため：

- **費用ゼロ・APIキー不要・通信不要**（端末にダウンロード済みの音声ならオフラインでも再生可）
- 読み上げ原稿は、取り込んだ問題の `explanation` / `choiceReasons` / メモから**アプリ内で組み立て**
- 誤答は `wrongLog` テーブルに1件ずつ記録（当日ぶん・直近30日ぶんを集計に使用。180日で自動間引き）

より自然な声にしたい場合は、端末側に日本語音声を追加します（iPhone: 設定＞アクセシビリティ＞
読み上げコンテンツ＞声／Android: 設定＞ユーザー補助＞テキスト読み上げ）。
なお解説文そのものをAIで補いたい場合は、従来どおり任意のAnthropic APIキー設定で
「AI解説」を生成し、その文章も音声で読み上げられます（キー未設定でも音声機能は使えます）。

## まとめノート（HTML学習資料）

「項目ごとに苦手だけ／項目まるごと」を印刷・保存できる形に整える機能です。生成は端末内で完結し、
出来上がるのは**CSS埋め込みの自己完結HTML1枚**（外部フォント・外部スクリプト・通信なし）。

- 対象の選び方: 全カテゴリ／7分類のいずれか × 「間違いが多い問題」「項目の全問題」
- 収録内容: 目次・正答率サマリ・各問（誤答回数バッジ／出典／選択肢／正解の理由／他肢が違う理由／メモ）
- 出力: アプリ内プレビュー（保存されるHTMLそのものをiframe表示）／HTMLファイル保存／印刷・PDF
- 印刷用CSS（`@media print`）で1問がページをまたがないように調整。ダークモードにも追従。
